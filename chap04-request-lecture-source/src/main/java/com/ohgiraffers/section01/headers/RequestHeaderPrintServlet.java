...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    *       (요청 응답 모두 사용되며, 메세지 바디의 컨텐츠를 나타내기에 GET요청은 해당되지 않는다.)
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // connection : HTTP 통신이 완료된 후에 네크워크 접속을 유지할지 결정 (기본값 keep-alive = 연결을 열린상태로 유지)
        ...
        // sec-fetch-user : 사용자가 시작한 요청일 때만 보내짐 (항상 ?! 값을 가짐)
        ...
        // upgrade-insecure-request : 클라이언트가 서버에세 http -> https 로 업그레이드를 요청할 때 사용
